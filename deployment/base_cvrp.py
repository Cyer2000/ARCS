...
def calFitness(birdPop, Demand, dis_matrix, CAPACITY, DISTANCE, C0, C1):
    ...
    输入：birdPop-路径，Demand-客户需求,dis_matrix-城市间距离矩阵，CAPACITY-车辆最大载重,DISTANCE-车辆最大行驶距离,C0-车辆启动成本,C1-车辆单位距离行驶成本；
    ...
    for j in range(len(birdPop)):
        ...
        while i < len(bird):
            if line == [0]:  # 车辆未分配客户点
                ...
                if (dis_matrix.loc[line[-1], bird[i]] + dis_matrix.loc[bird[i], 0] + dis <= DISTANCE) & (
...
def main(Customer, Demand, CAPACITY, DISTANCE):
    # 车辆参数
    # CAPACITY = 120  # 车辆最大容量
    # DISTANCE = 8500  # 车辆最大行驶距离
    C0 = 0  # 无车辆启动成本
    C1 = 1  # 车辆单位距离行驶成本
    ...
    # Demand = [0, 16, 11, 6, 10, 7, 12, 16, 6, 16, 8, 14, 7, 16, 3, 22, 18, 19, 1, 14, 8, 12, 4, 8, 24, 24, 2, 10,
    #           15, 2, 14, 9]
    ...
    birdPop_car, fits = calFitness(birdPop, Demand, dis_matrix, CAPACITY, DISTANCE, C0, C1)  # 分配车辆，计算种群适应度
